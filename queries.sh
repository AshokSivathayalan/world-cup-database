...
#Creating PSQL variable to interact with database
PSQL="psql --username=freecodecamp --dbname=worldcup --no-align --tuples-only -c"

#Making various queries to database and outputting results
...
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name ASC;")"
